...
| `bind_logging` | `dict` | `false` | {} | BIND logging configuration with channels and categories.<br>Channel and category names are mapping keys and therefore unique.<br>An empty mapping renders the platform package logging configuration.<br>A user-defined mapping replaces the complete platform logging configuration. |
